...
import br.ufc.great.syssu.net.UdpResponser;
...
public class UbiCentreProcess implements Runnable {
	...
	public void run() {
		boolean isRunningOnServer = System.getProperty("java.vm.name").contains("HotSpot");

		if (server != null) {
			try {
				if(isRunningOnServer) {
					Thread t = new Thread(new UdpResponser());
					t.start();
				}
				server.start();
